                    ...
                    <table class="table">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>Tracking Url</td>
                                <td>
                                    <input id="orderTracking" type="text" name="tracking_url" value="{{$order->tracking_url}}" data-id="{{$order->id}}">
                                </td>
                                <script src="https://code.jquery.com/jquery-3.6.1.min.js" integrity="sha256-o88AwQnZB+VDvE9tvIXrMQaPlFFSUTR+nldQm1LuPXQ=" crossorigin="anonymous"></script>
                                <script>
                                   $('#orderTracking').change(function() {
                                        var orderId = $(this).attr('data-id');
                                        var value = $(this).val();
                                        $.ajax({
                                            type: 'PUT',
                                            url: route('admin.orders.tracking', orderId),
                                            data: { tracking_url: value},
                                            success: (message) => {
                                                success(message);
                                            },
                                            error: (xhr) => {
                            ...
                            <tr>
                                ...
                                <td>
                                    <input id="orderTrackingNumber" type="text" name="tracking_url" value="{{$order->tracking_number}}" data-id="{{$order->id}}">
                                </td>
                                <script>
                                   $('#orderTrackingNumber').change(function() {
                                        var orderId = $(this).attr('data-id');
                                        var value = $(this).val();
                                        $.ajax({
                                            type: 'PUT',
